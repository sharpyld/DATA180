...
```{r}
Vans_dist <- dist(Vans93[,2:6])
Vans_dist
Vans_Euclidean <- as.matrix(Vans_dist)
...
# max_dist = (Lumina_APV, Eurovan) ; min_dist = (Caravan, Aerostar)
```
...
```{r}
Vans_dist2 <- dist(Vans93[,2:6], method = "manhattan")
Vans_dist2
Vans_Manhattan <- as.matrix(Vans_dist2)
...
d^s(A,B) = min(d($O_1$,$O_2$),d($O_1$,$O_3$),d($O_1$,$O_5$),d($O_4$,$O_2$),d($O_4$,$O_3$),d($O_4$,$O_5$)) = min(1,9,1.6,6,5,7.5)= 1


b) Farthest-neighbor


d^C(A,B)= (d($O_1$,$O_2$),d($O_1$,$O_3$),d($O_1$,$O_5$),d($O_4$,$O_2$),d($O_4$,$O_3$),d($O_4$,$O_5$))= max(1,9,1.6,6,5,7.5)= 9


c) Average-neighbor

d^A(A,B)= 1/(3x2) x (d($O_1$,$O_2$),d($O_1$,$O_3$),d($O_1$,$O_5$),d($O_4$,$O_2$),d($O_4$,$O_3$),d($O_4$,$O_5$)) = 1/(3x2) x (1,9,1.6,6,5,7.5)= 30.1/6= 5.02 (-2)
...
$C_5 = \{as\}$
```{r}
# The grouping is based on the split into sub-groups of words on the level of roughly half of the total height of the dendrogram.
```
...
```{r}
#vertebrate <- read.csv("/Users/dsharpylo/Documents/RStudio/DATA180_Dickinson/DATA180/hw5/vertebrate.csv")
...
plot(hclust(Ver_Euclidean, method = "complete"), lwd=3, main= "Clustered vertebrates, Euclidean type,\n Complete type" , sub="", xlab= "Vertebrates", cex.lab=1.25, hang=-1)
```
...
plot(hclust(Ver_Manhattan, method = "single"), lwd=3, main= "Clustered vertebrates, Manhattan type,\n Single type" , sub="", xlab= "Vertebrates", cex.lab=1.25, hang=-1)
```

Which method do you prefer?

- I think the single linkage turned out to be a "prettier" or a more structured dendrogram than the complete one (in my opinion), so I would prefer the single method. 
...
```{r}
library(ggdendro)
ggdendrogram(hclust(Ver_Manhattan), segments = TRUE, labels = TRUE, rotate = FALSE, size=2)
```
